...
public abstract class OldArbCase extends AbstractJob {
	...
	// ---------------- Define Case Interface and abstract method ----------------
		 ...
		 * 
		 */
		public abstract ArbCaseOld getArbCaseImplementation();
		...
		public static interface ArbCaseOld {
		...
		private IDB teamDB;
		private ArbCaseOld implementation;
		...
		public void processQueue(int tick){
			while (!this.queue.isEmpty() && this.queue.peek().deliveryTick >= tick){
				...
				if(event instanceof DelayedOrderFill){
					DelayedOrderFill fill = (DelayedOrderFill) event;
					//implementation.fillNotice(fill.exchange, fill.price, fill.algoside);
				}
				else if(event instanceof DelayedTopOfBook){
